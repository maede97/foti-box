...
export default function AdminPage() {
  ...
  async function handleAddEvent() {
    if (!eventName || !eventPassword)
      return setError("Provide name and password");
      ...
      <section style={{ marginBottom: "2rem" }}>
        ...
        {events.map((evt) => (
          <div key={evt._id} style={{ marginBottom: "0.5rem" }}>
            <span style={{ fontWeight: evt.active ? "bold" : "normal" }}>
              {evt.name}
            </span>
            {!evt.active && (
              <button
            ...
            {evt.active && (
              <span style={{ marginLeft: "1rem", color: "green" }}>Active</span>
            )}
          </div>
        ))}
      </section>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem" }}>
            {images.map((img) => (
              <div key={img.uuid} style={{ position: "relative" }}>
                <a href={`/gallery/${img.uuid}`} target="_blank">
                  <img
