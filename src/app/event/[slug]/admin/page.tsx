...
import EventAdminPageClient from '@/pages/event-admin-client';
...
export const generateMetadata = async ({ params }: { params: ParamsType }) => {
  ...
  return {
    ...
    description: `foti-box.com | Admin für ${name}`,
  };
};
...
const EventPage: React.FC<{ params: ParamsType }> = async ({ params }) => {
  ...
  const imageCount = await Image.countDocuments({ event: dbEvent._id });

  return (
    <EventAdminPageClient
